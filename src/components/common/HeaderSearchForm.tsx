        ...
        <div className='w-full'>
            <form className=' relative relative mr-2 w-full' onSubmit={e => handleSearchSubmit(e)}>
                <input
                    ...
                    className='mr-2 w-full rounded border border-gray-300 p-2 px-5 focus:border-primary focus:outline-none'
                />
                {/* on show reset, if user has searched previously */}
                {searchTerm && (
                    <button
                        title='reset search filter'
                        className='absolute right-3 top-[50%] -translate-y-1/2 rounded bg-danger-light px-2 py-1 text-sm text-white hover:bg-danger '
                        type='button'
                        onClick={handleReset}
                    >
                        Reset
            ...
            >
                {searchProducts && searchProducts?.data.length > 0 ? (
                    searchProducts?.data.map((prod: Product, index: number) => {
                        return (
